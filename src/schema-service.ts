...
export class SchemaService {
  ...
  transform(
    fn: (key: string, value: ApiSchema) => ApiSchema,
    path: string = "entity"
  ) {
  ...
  get<T>(
    ClassOrString: Class<T> | string,
    path: string = "entity"
  ): ApiSchema<T> {
    if (typeof ClassOrString == "string")
      return this.schemas[path]![ClassOrString];
    else return this.schemas[path]![ClassOrString.name];
  }
  ...
  defineAllSchemas(target: Class, path: string = "entity", prefix = "") {
    for (const key in this.schemas[path]) {
      ApiDefineSchema(prefix + key, this.schemas[path][key])(target);
    }
  }
}
